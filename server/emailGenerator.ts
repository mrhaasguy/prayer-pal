...
export async function generateDailyPrayersTemplate(userEmail: {userId: string, fullName: string}, dalService: IDalService) {
      ...
      return Mustache.render(html, {
        user: { name: parseFullName(userEmail.fullName).first ?? userEmail.fullName, id: userEmail.userId },
        date: new Date(),
        prayerRequests,
        index: () => (index += 1),
        FormatDate: function () {
